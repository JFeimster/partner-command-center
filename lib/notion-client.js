...
async function upsertPartner(partner) {
  const existing = partner.partner_id
    ? await findPartnerByPartnerId(partner.partner_id)
    : partner.email
      ? await findPartnerByEmail(partner.email)
      : null;

  if (existing && existing.id) {
    return {
      action: 'updated',
      page: await updatePartner(existing.id, partner),
  ...
  return {
    action: 'created',
    page: await createPartner(partner)
  };
}
