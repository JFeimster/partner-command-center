...
function multiSelectFromText(value) {
  return multiSelect(splitToOptions(value));
}
...
function partnerToNotionProperties(partnerInput) {
  ...
  return {
    ...
    'Audience': multiSelectFromText(partner.audience),
    'Referral Source': select(partner.referral_source),
    'Traffic Source': select(partner.traffic_source),
