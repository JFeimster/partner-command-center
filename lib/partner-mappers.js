...
function richText(content) {
  ...
  if (!normalized) return { rich_text: [] };
  const chunks = normalized.match(/[\s\S]{1,1900}/g) || [];
  return {
...
function normalizePartner(input) {
  ...
  return {
    ...
    sales_experience: cleanString(input.sales_experience),
    self_description: cleanString(input.self_description).replace(/[‘’]/g, "'"),
    wants_strategy_call: cleanString(input.wants_strategy_call),
    interest_reason: cleanString(input.interest_reason),
    preferred_start: cleanString(input.preferred_start),
    source_form: cleanString(input.source_form),
...
function partnerToNotionProperties(partnerInput) {
  ...
  return {
    ...
    ...(partner.current_position ? { 'Current Position': richText(partner.current_position) } : {}),
    ...(partner.sales_experience ? { 'Sales Experience': select(partner.sales_experience) } : {}),
    ...(partner.self_description ? { 'Self Description': select(partner.self_description) } : {}),
    ...(partner.wants_strategy_call ? { 'Wants Strategy Call': select(partner.wants_strategy_call) } : {}),
    ...(partner.interest_reason ? { 'Interest Reason': richText(partner.interest_reason) } : {}),
    ...(partner.preferred_start ? { 'Preferred Start': select(partner.preferred_start) } : {}),
    ...(partner.source_form ? { 'Source Form': richText(partner.source_form) } : {}),
    ...(partner.notes ? { 'Notes': richText(partner.notes) } : {})
  };
}
...
function notionEventType(value) {
  const key = normalizedKey(value);
  if (key === 'partner_enrollment_notification' || key === 'partner_signup_created' || key === 'intake_submitted') return 'Intake Submitted';
  if (key === 'status_updated') return 'Status Updated';
  if (key === 'resource_recommended') return 'Resource Recommended';
  if (key === 'campaign_recommended') return 'Campaign Recommended';
  if (key === 'tracking_link_created') return 'Tracking Link Created';
  if (key === 'email_sent') return 'Email Sent';
  if (key === 'manual_review_requested') return 'Manual Review Requested';
  if (key === 'crm_sync_attempted') return 'CRM Sync Attempted';
  if (key === 'crm_sync_failed') return 'CRM Sync Failed';
  if (key === 'crm_sync_completed') return 'CRM Sync Completed';
  return 'Note Added';
}

function notionEventSource(value) {
  const key = normalizedKey(value);
  if (key === 'email') return 'Email';
  if (key === 'tally') return 'Tally';
  if (key === 'gpt_action') return 'gpt_action';
...
function notionEventStatus(value) {
  const key = normalizedKey(value);
  if (['new', 'intake_received', 'needs_review', 'approved', 'active', 'paused', 'rejected', 'archived'].includes(key)) return key;
  if (key === 'received' || key === 'accepted') return 'intake_received';
  return 'new';
}

function partnerEventToNotionProperties(eventInput) {
  requireServerSide();
  const occurredAt = eventInput.created_at || nowIso();
  ...
  return {
    'Event Name': title(eventId),
    'Partner ID': richText(eventInput.partner_id),
    'Event Type': select(notionEventType(eventInput.event_type)),
    'Source': select(notionEventSource(eventInput.source)),
    'Status': select(notionEventStatus(eventInput.status)),
    'Summary': richText(eventInput.summary),
    'Event Summary': richText(eventInput.summary),
    'Metadata JSON': richText(safeJson({
      event_id: eventId,
      ...(eventInput.metadata && typeof eventInput.metadata === 'object' ? eventInput.metadata : {})
    })),
    'Occurred At': date(occurredAt)
  };
}
