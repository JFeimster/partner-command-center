...
function parseApplicant(input) {
  ...
  const revenueMatch = text.match(/lowest monthly revenue[^$]*\$([\d,]+(?:\.\d{1,2})?)/i);
  const revenue = extractLabeled(text, ['monthly revenue', 'revenue']) || (revenueMatch ? revenueMatch[1] : '');
  ...
  const bankAccountType = bankMatch ? bankMatch[1].trim() : '';
