...
function applicantDealStage(status) {
  ...
  if (/closed|lost|withdrawn/.test(value)) return 'closedlost';
...
async function upsertApplicantDeal(applicant, contactId) {
  ...
  return { configured: true, status, deal_id: deal.id || (existing && existing.id), contact_association: association };
}
